...
#ip <- data.frame(table(df$userid))
#ip <- ip[ip$Freq==1,]
#nu <- du[du$ip %in% ip$Var1[ip$Freq>1],]

##### IP addresses? ################

x <- View(data.frame(sort(table(du$ip))))
y <- data.frame(du$ip, as.integer(x[du$ip]))
colnames(y) <- c('IP', 'occurrences')
repeats <- y$IP[y$occurrences > 1]

arr <- arrange(du, ip, age, playerid) #%>%
#subset(arr, ip %in% )
ar <- select(arr, ip, age, nationality, occupation, app, playerid,
             gender, offer, accept, mTurkCode,
             participant.mturk_worker_id
)
View(ar)

ar2 <- arrange(du, ip)
View(select(ar2, ip, LocationLatitude, LocationLongitude))

#### IP check pt. II ############
ip <- data.frame(table(du$ip))
ip <- ip[ip$Freq>1,]
nu <- du[du$ip %in% ip$Var1[ip$Freq>1],]

long <- data.frame(table(du$LocationLongitude))
long <- long[long$Freq>1,]
nu <- nu[nu$LocationLongitude %in% long$Var1[long$Freq>1],]

lat <- data.frame(table(du$LocationLatitude))
lat <- lat[lat$Freq>1,]
nu <- nu[nu$LocationLatitude %in% lat$Var1[lat$Freq>1],]

rep.ip <- du$ip[du$ip %in% ip$Var1[ip$Freq>1]]
rep.long <- du$LocationLongitude[du$LocationLongitude %in% long$Var1[long$Freq>1]]
rep.lat <- du$LocationLatitude[du$LocationLatitude %in% lat$Var1[lat$Freq>1]]

nu <- subset(du, LocationLatitude %in% rep.lat & LocationLongitude %in% rep.long)
nu <- arrange(nu, ip)
View(select(nu, ip, LocationLatitude, LocationLongitude, participant.mturk_worker_id))
...
reps <- c('A9AJUQKWN5LVJ',
          'A4KSSWL5Z1V2L',
          'A19F75QCU199K8',
          'A2SBISQJE2ZPJQ',
          'A3SOJWB6AZWZV9',
          'AK467M5VTG7N4')
View(subset(du, participant.mturk_worker_id %in% reps))
